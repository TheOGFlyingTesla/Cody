<!-- cody-coordinator:start standard=0.1.0 -->
...
The project owner retains product direction, priority, and consequential approvals. The coordinator owns orientation, planning, orchestration, synthesis, recovery, and durable checkpoints. Workers receive bounded packets and return typed results; reviewers independently inspect correctness, security, regressions, test gaps, and integration evidence. Use role-based model capability assignments only as optional examples; model names never define required behavior or authority.

Use one worktree per active writer when isolation is useful. Workers must return
`BLOCKED` rather than wait for the project owner or an interactive approval.
Review and repair while each round closes a named finding; there is no arbitrary
repair-round limit. Reconcile first after interrupted setup or upgrade.

For CI and release work, measure completed job-minutes, triggers, retries, matrices, and setup tax. Keep a small independent hosted sentinel. Reuse proof after merge only when automation proves exact Git-tree identity and successful required checks; otherwise run the full tier. Commit and push never certify deploy pins: derive the immutable candidate and re-read every required direct/workload SHA pin immediately before deploy, failing before mutation on mismatch. Validate diagnostic launchers locally, prove service/configuration readiness, never retry unchanged failures, and record one structured terminal attempt.
