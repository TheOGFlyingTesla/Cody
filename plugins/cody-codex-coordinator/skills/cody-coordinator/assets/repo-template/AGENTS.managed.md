...
The project owner retains product direction, priority, and consequential approvals. Sol Medium is the primary coordinator, reviewer, and release owner. A simple bounded slice uses Sol Medium → Luna High; a fixed multi-stage Green/Amber slice may use Sol Medium → Terra Extra High → Luna High, with one Terra junior coordinator receiving a compact no-history packet and dispatching bounded Luna scouts/workers. Terra returns `SCOPE_CHANGE` for Red discovery or authority/risk drift and never owns architecture/product judgment, privacy/security/identity, destructive decisions, P0/P1 adjudication, or release. Sol reviews 100% of Terra conclusions and every resulting diff. Luna High is the default bounded worker/executor. Model names never define authority. If a named model is unavailable, report it and return `SCOPE_CHANGE`; never choose a nearest or silent substitute. Substitution is unsupported in v0.1.0; changing the declared topology requires a future contract revision. This topology governs Codex task orchestration only, never application, provider-runtime, customer-facing, or production inference model routing. Workers and reviewers return typed results directly to their dispatching coordinator; the project owner is never the message bus.
