...
| Windows | Read-only preview | Inspection is exercised in CI; coordinator mutation, secure bundle extraction, and user-scoped skill installation/discovery-path verification fail closed because the required descriptor primitives are unavailable. |
...
The installation scope is the documented user-scoped
`$HOME/.agents/skills/cody-coordinator` path on supported Unix-like platforms.
Repository-scoped skill installation is not created by the installer.
